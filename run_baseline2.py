...
def train(num_timesteps, seed, model_path=None, env_id=None):
    ...
    def policy_fn(name, ob_space, ac_space):
        return mlp_policy.MlpPolicy(name=name, ob_space=ob_space, ac_space=ac_s$
    ...
    print(" ts "*50,num_timesteps," ts"*50)
...
def main():
    ...
    parser.add_argument('--model-path', default=os.path.join('/root/code/nti/gy$
    ...
    if False or not args.play:
        # train the model
        train(num_timesteps=args.num_timesteps, seed=args.seed, model_path=args$
    else:
        print("$"*200)
  ...
  #      import pickle
 #        with open("model.jbl","wb") as f:
  #           joblib.dump(pi,f)
        print(args.model_path)
        U.load_state(args.model_path)
        seeds=[3717,1515,2515]
